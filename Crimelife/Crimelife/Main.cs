...
namespace Crimelife
{
    public class Main : Script
    {
        ...
        public void InitGameMode()
        {
            ...
            Logger.Print("");
            Logger.Print("1.1 Started. lg von rojas#1337 und Valito#7291");
            Logger.Print("");

            MySqlHandler.ExecuteSync(new MySqlQuery("UPDATE vehicles SET Parked = 1"));
            Logger.Print("Autos eingeparkt!");
        ...
        public static void OnHourHandler()
        {
            try
            {
                ...
                foreach (DbPlayer dbPlayer in PlayerHandler.GetPlayers())
                {
                    ...
                    dbPlayer.SendNotification("Sie haben ihren Payday erhalten! +50.000$", 5000, "darkgreen", "KONTOVERAENDERUNG");
                    Adminrank adminranks = dbPlayer.Adminrank;

                    if (adminranks.Permission >= 91)
                    {
                        dbPlayer.SendNotification("Da du ein Teamler bekommst du einen extra PayDay! +50.000$", 5000, "red", "KONTOVERAENDERUNG");
                        dbPlayer.addMoney(50000);
                    }
                }
            }
