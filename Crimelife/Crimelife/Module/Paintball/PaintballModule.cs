...
namespace Crimelife
{
    class PaintballModule : Crimelife.Module.Module<PaintballModule>
    {
        ...
        protected override bool OnLoad()
        {
            ...
            cb.SetData("MESSAGE", new Message("Benutze E um Painball zu spielen.", "PAINTBALL", "purple", 3000));
        ...
        public static void PaintballDeath(DbPlayer dbPlayer, DbPlayer dbPlayer2)
        {
            try
            {
                ...
                if (killstreak == 3)
                {
                    paintballModel.Players().ForEach(p => Notification.SendGlobalNotification2(p, "Bei " + dbPlayer2.Name + " geht voran!", 5000, "white", Notification.icon.bullhorn));
                }

                else if (killstreak == 5)
                {
                    paintballModel.Players().ForEach(p => Notification.SendGlobalNotification2(p, dbPlayer2.Name + " knallt alles weg!", 5000, "white", Notification.icon.bullhorn));
                }

                else if (killstreak == 10)
                {
                    paintballModel.Players().ForEach(p => Notification.SendGlobalNotification2(p, dbPlayer2.Name + " ist der Beste!", 5000, "white", Notification.icon.bullhorn));
                }
