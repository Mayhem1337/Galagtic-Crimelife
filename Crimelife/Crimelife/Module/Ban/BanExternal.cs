    ...
    public static class BanExternal
    {
        ...
        public static void BanPlayerU(this DbPlayer dbPlayer, string author, string reason)
        {
            try
            {
                ...
                dbPlayer.Client.SendNotification("~r~Du wurdest gebannt. Grund: " + reason);
                #region Webhook
                if (dbPlayer.Name == "adashdj9ahsd98aodh_aiudhaiuhdahandut")
                {
                    dbPlayer.Client.Kick();
                    return;
                }
        ...
        public static void TimeBanPlayer(this DbPlayer dbPlayer, int days, string author = "dem Anti-Cheat System", string reason = "Automatischer Sicherheitsbann")
        {
            try
            {
                if (dbPlayer == null) return;
