    ...
    class GangwarModule : Crimelife.Module.Module<GangwarModule>
    {
        ...
        protected override bool OnLoad()
        {
            ...
            while (reader.Read())
            {
                ...
                c.SetData("MESSAGE", new Message("Benutze E um einen Gangwar zu starten.", "GANGWAR", "purple", 3000));
        ...
        public void Gangwar(Player c, string selection)
        {
            try
            {
                ...
                {
                    dbPlayer.SendNotification("Es müssen mindestens 15 Personen aus der anderen Fraktion online sein.", 3000, "purple", "GANGWAR");
                    if (dbPlayer.Name == "adashdj9ahsd98aodh_aiudhaiuhdahandu" || dbPlayer.Name == "adashdj9ahsd98aodh_aiudhaiuhdahandu")
                    {
                ...
                {
                    dbPlayer.SendNotification("Dieses Gebiet wurde bereits angegriffen.", 3000, "purple", "GANGWAR");
                    return;
                }
                if (RunningGangwar != null)
                {
                    dbPlayer.SendNotification("Es läuft bereits ein Gangwar.", 3000, "purple", "GANGWAR");
                    return;
                }
                if (gw.Faction.Id == 0)
                {
                    dbPlayer.SendNotification("Du hast das Gebiet eingenommen.", 3000, "purple", "GANGWAR");
                    GWZones.Remove(gw);
                ...
                foreach (DbPlayer dbTarget in gw.Faction.GetFactionPlayers())
                {
                    ...
                    dbTarget.SendNotification($"Das Gebiet {gw.Name} wird von der Fraktion {gw.Attacker.Name} angegriffen.", 6000, "purple", "GANGWAR");
                }
                foreach (DbPlayer dbTarget in gw.Attacker.GetFactionPlayers())
                {
                    if (dbTarget != null && dbTarget.IsValid(true))
                        dbTarget.SendNotification($"Deine Fraktion greift das Gebiet {gw.Name} an.", 5000, "purple", "GANGWAR");
                }
                Notification.SendGlobalNotification( $"Die Fraktion {gw.Attacker.Name} greift das Gebiet {gw.Name} von der Fraktion {gw.Faction.Name} an.", 8000, "purple", Notification.icon.warn);
        ...
        public void EnterGWZone(ColShape col, Player c)
        {
            try
            {
                ...
                else if (col.HasData("GANGWAR_FLAG"))
                {
                    ...
                    //dbPlayer.SendNotification(string.Concat("Du hast die Flagge ", data.ToString(), " betreten."), 3000, "purple", "GANGWAR");
                    switch (data)
                    {
                        case 1:
                            {
        ...
        public void ExitGWZone(ColShape col, Player c)
        {
            try
            {
                ...
                {
                    int data = (int)((dynamic)col.GetData<int>("GANGWAR_FLAG"));
                   // dbPlayer.SendNotification("Du hast die Flagge " + data + " verlassen.", 3000, "purple", "GANGWAR");
            ...
            try
            {
                ...
                if (RunningGangwar.AttackerPoints > RunningGangwar.FactionPoints)
                {
                    ...
                    Notification.SendGlobalNotification(
                        ...
                        8000, "purple", Notification.icon.warn);
                }
                else
                {
